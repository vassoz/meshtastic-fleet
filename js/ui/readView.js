  ...
  switch (action) {
    ...
    case "save-snapshot": {
      ...
      const id = state.liveSnapshot.nodeNum != null
        ...
        : `snap-${Date.now()}`;
      const defaultLabel = connectionLabel(state.connection, state.liveSnapshot) ?? `Snapshot ${formatTimestamp(new Date().toISOString())}`;
    ...
    case "save-local-from-read": {
      ...
      profile.boundTo = { nodeNum: snap.nodeNum, bleName: connectionLabel(state.connection, state.liveSnapshot), hwModel: snap.hwModel ?? null };
      touch(profile);
